...
const AuthModal: React.FC<AuthModalProps> = ({ onClose, onSuccess, inline, initialIsSignUp = true }) => {
  ...
  const content = (
      <div className={`w-full max-w-sm bg-white rounded-[2.5rem] p-8 ${inline ? '' : 'shadow-2xl relative overflow-hidden'}`}>
        {!inline && onClose && (
          <button type="button" onClick={onClose} className="absolute top-6 right-6 text-theme-primary/40 sm:hover:text-theme-primary/70">
        ...
        <div className="text-center mb-8">
           ...
           <h2 className="text-2xl font-display font-black italic text-text-main">{isSignUp ? 'Join the Field' : 'Welcome Back'}</h2>
           <p className="text-theme-primary/60 text-xs mt-2">Sign in to publish your discoveries.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
           ...
           {/* Helper text for indicators */}
            {isSignUp && (
                <div className="flex justify-between text-[9px] text-theme-primary/40 uppercase font-bold tracking-wider px-1">
                    <span className={hasMinLength ? 'text-accent' : ''}>6+ Chars</span>
                    <span className={hasNumber ? 'text-accent' : ''}>Number</span>
                    <span className={hasSpecial ? 'text-accent' : ''}>Symbol</span>
                </div>
            )}
        ...
        <div className="mt-6 text-center">
          <button 
            ...
            className="text-xs text-theme-primary/60 font-bold sm:hover:text-primary transition-colors"
          >
